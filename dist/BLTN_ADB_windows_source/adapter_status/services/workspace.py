...
from ..processes import new_process_group_kwargs, register_process
...
def adb_script_candidates():
    if IS_WINDOWS:
        return [
            os.path.join(docs_dir(), "adb_windows.cmd"),
            os.path.join(project_workdir(), "adb_windows.cmd"),
        ]
    return [
...
def open_terminal_session():
    ...
    try:
        ...
        popen_kwargs = new_process_group_kwargs()
        if IS_WINDOWS:
        ...
        proc = subprocess.Popen(command, cwd=workdir, env=env, **popen_kwargs)

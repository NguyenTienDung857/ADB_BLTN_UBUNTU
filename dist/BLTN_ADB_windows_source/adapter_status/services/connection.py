...
from ..host_platform import interface_status, ping_command
...
def adb_reconnect(config):
    ...
    while time.monotonic() < deadline:
        ...
        if now >= next_ping_at:
            ping_code, _ping_output = run(ping_command(config["device_ip"]), timeout=2, purpose="adb-reconnect-ping")
...
def collect_status(config):
    ...
    host_status = interface_status(config)
    iface = host_status["iface"]
    exists = bool(host_status["exists"])
    carrier = host_status.get("carrier")
    operstate = host_status.get("operstate") or "missing"
    ip_output = host_status.get("ip_output") or ""
    has_expected_ip = bool(host_status.get("has_expected_ip"))

    ping_ok = False
    ping_output = "Bỏ qua vì cổng chưa sẵn sàng."
    if exists and carrier == "1" and has_expected_ip:
        ping_code, ping_output = run(ping_command(device_ip), timeout=2, purpose="status")
        ping_ok = ping_code == 0

...
4. Host platform layer: chọn lệnh và đường dẫn đúng cho Ubuntu hoặc Windows.

UI không gọi `adb`, `netsh`, `ip` hoặc `subprocess` trực tiếp. UI gọi service, service gọi host platform/executor.
...
├── run_windows.cmd                   # entrypoint chạy app trên Windows
├── bootstrap_windows.cmd             # cài runtime MSYS2 GTK/Python và psutil
├── check_windows_runtime.cmd         # preflight runtime Windows
├── smoke_windows.cmd                 # smoke test app/runtime không cần thiết bị
├── validate_windows_with_device.cmd  # kiểm kết nối thật với BLTN trên Windows
├── scripts/
│   ├── package_windows.sh            # rebuild zip Windows từ Ubuntu
│   └── verify_windows_package.py     # kiểm zip không thiếu file/generated/private key
├── tools/
│   ├── platform-tools/               # vị trí portable adb.exe nếu cần
│   └── mpv/                          # vị trí portable mpv.exe nếu cần
├── adapter_status/
...
│   ├── host_platform.py              # khác biệt Ubuntu/Windows: config dir, netsh/ip, ping, adb.exe, mpv
│   ├── windows_preflight.py          # kiểm runtime/layout Windows trước khi mở app
│   ├── windows_smoke.py              # smoke test non-destructive cho package Windows
│   ├── windows_device_validation.py  # validation non-destructive với BLTN thật
│   ├── windows_adb_shell.py          # backend cho docs/adb_windows.cmd
...
: Boundary cho khác biệt host OS. Linux dùng `/sys`, `ip`, `ping -c`; Windows dùng `%APPDATA%`, `%LOCALAPPDATA%`, PowerShell `Get-NetAdapter`, `netsh`, `ping -n`, `adb.exe`, `taskkill` và `os.pathsep` cho `ADB_VENDOR_KEYS`.

`adapter_status/processes.py`
: Theo dõi process do app mở, cleanup process treo, kill theo process group trên Linux hoặc `taskkill` trên Windows, chống kill nhầm process khác bằng `pid + start_time/create_time`.
...
services/* xử lý nghiệp vụ và gọi adb/executor.py
    ↓
host_platform chọn lệnh host OS, executor chạy subprocess có timeout/process tracking
    ↓

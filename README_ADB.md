...
Nút này chỉ sáng khi app thấy ADB đã lên `device`.

- Nếu ADB đã connect nhưng chưa có root, nút màu đỏ và hiện `Get Root`.
- Nếu thiết bị đã có `uid=0(root)`, nút màu xanh và hiện `Root OK`.

Khi bấm `Get Root`, app mở cửa sổ chọn file debug `.bin`; chọn đúng file dạng `DEBUG_...bin` dùng cho build đang chạy trên thiết bị.

Sau khi chọn file, app tự chạy quy trình lấy root và in log từng bước trong khung log:

- Kiểm tra `adb devices` và `adb shell id`.
- Nếu thiết bị đã là `uid=0(root)`, app dừng an toàn và báo đã có root.
- Push file debug vào `/tmp/cpu_update.bin`.
- Chạy `cmdtool` với lệnh `update cpu`.
- Chạy `cd /home/adb && ./change_file`.
- Thiết bị tự reboot sau `change_file`; app chờ, tự connect lại ADB và kiểm tra `adb shell id`.

Kết quả thành công là log cuối có `uid=0(root)`. Trong lúc Get Root đang chạy, app khóa các nút thao tác ADB khác để tránh chạy chồng lệnh.
